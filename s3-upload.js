...
const FOLDER = process.env.S3_FOLDER1;
...
function resolveCloudfrontHostname(value) {
...
// Build the current year+month prefix (e.g. "202607") used to bucket uploads by month
...
// Build the S3 key for a given file name: FOLDER/YYYYMM/fileName
function buildKey(fileName) {
  return `${FOLDER}/${getYearMonthPrefix()}/${fileName}`;
}
...
async function uploadFile(fileName) {
  ...
  const hostname = resolveCloudfrontHostname(CLOUDFRONT_URL) || `${BUCKET}.s3.${REGION}.amazonaws.com`;

  return {
    s3Uri: `s3://${BUCKET}/${key}`,
    url: `https://${hostname}/${encodedKey}`,
  };
}
...
async function main() {
  ...
  const csvWriter = createObjectCsvWriter({
    path: path.join(__dirname, 'uploads.csv'),
    header: [
